...
jika sys.version_info.major <= 2: import httplib
lain: impor http.client sebagai httplib
...
versionPath = "inti" + os.sep + "version.txt"

errMsg = lambda msg: write (rd + "\ n [" + yl + "!" + rd + "] Error:" + yl + msg + rd + "!!! \ n" + wi)

coba: permintaan impor
kecuali ImportError:
    errMsg ("modul [permintaan] hilang")
    print ("[*] Harap Gunakan: 'permintaan pemasangan pip' untuk menginstalnya :)")
    sys.exit (1)

coba: impor mekanik
kecuali ImportError:
    errMsg ("modul [mekanik] hilang")
    print ("[*] Silakan Gunakan: 'pip install mechanize' untuk menginstalnya :)")
    sys.exit (1)

kelas FaceBoom (objek):


    def __init __ (sendiri):
        self.useProxy = Tidak Ada
        self.br = mechanize.Browser ()
        self.br.set_handle_robots (Salah)
        self.br._factory.is_html = Benar
        self.br.addheaders = [('User-agent', random.choice ([
               'Mozilla / 5.0 (Windows NT 6.1; WOW64) AppleWebKit / 534.24 (KHTML, seperti Gecko) RockMelt / 0.9.58.494 Chrome / 11.0.696.71 Safari / 534.24',
               'Mozilla / 5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit / 537.36 (KHTML, seperti Gecko) Chrome / 35.0.1916.47 Safari / 537.36',
               'Mozilla / 5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit / 535.2 (KHTML, seperti Gecko) Chrome / 15.0.874.54 Safari / 535.2',
               'Opera / 9.80 (J2ME / MIDP; Opera Mini / 9.80 (S60; SymbOS; Opera Mobi / 23.348; U; en) Presto / 2.5.25 Versi / 10.54',
               'Mozilla / 5.0 (X11; Linux x86_64) AppleWebKit / 535.11 (KHTML, seperti Gecko) Chrome / 17.0.963.12 Safari / 535.11',
               'Mozilla / 5.0 (Windows NT 5.1) AppleWebKit / 535.6 (KHTML, seperti Gecko) Chrome / 16.0.897.0 Safari / 535.6',
    ...
    def check_proxy (proxy):
          ...
          mencoba:
            ...
            jika proxy_ip == r.headers ['X-Client-IP']: mengembalikan True
            return False
          kecuali Exception: return False
    ...
    def cnet ():
        mencoba:
            ...
            kembali Benar
        kecuali socket.error: pass
        return False


    def get_profile_id (self, target_profile):
        mencoba:
            cetak (gr + "\ n [" + wi + "*" + gr + "] mendapatkan Id Profil target ... harap tunggu" + wi)
            idre = re.compile ('"entity_id": "([0-9] +)"')
            con = requests.get (target_profile) .text
            idis = idre.findall (con)
            cetak (wi + "\ n [" + gr + "+" + wi + "]" + gr + "Profil Target" + wi + "ID:" + yl + idis [0] + wi)
        kecuali IndexError:
